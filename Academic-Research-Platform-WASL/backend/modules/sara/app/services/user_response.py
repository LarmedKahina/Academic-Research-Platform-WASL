...
def ensure_profile_for_user(db: Session, u: User) -> bool:
    """
    Create an empty role profile row if the user should have one but it is missing
    (e.g. legacy data or failed signup). Caller should commit when True is returned.
    """
    if u.role == "admin":
        return False
    if u.role == "student":
        if db.scalars(select(StudentProfile).where(StudentProfile.user_id == u.id)).first():
            return False
        db.add(StudentProfile(user_id=u.id, university=None, department=None))
        return True
    if u.role == "professor":
        if db.scalars(select(ProfessorProfile).where(ProfessorProfile.user_id == u.id)).first():
            return False
        db.add(ProfessorProfile(user_id=u.id, university=None, department=None, title=None))
        return True
    if u.role == "company":
        if db.scalars(select(CompanyProfile).where(CompanyProfile.user_id == u.id)).first():
            return False
        db.add(CompanyProfile(user_id=u.id, company_name=None, industry=None))
        return True
    return False
