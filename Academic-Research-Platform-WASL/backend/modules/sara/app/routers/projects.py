...
from app.deps import get_current_user, get_optional_user, get_verified_user
...
def create_project(
    ...
    print(f"DEBUG: Project creation request from user {current.id}, role={current.role}")
    print(f"DEBUG: Body={body.model_dump()}")
    ...
    p = Project(
        ...
        # Student submissions require admin approval before public listing.
        status="pending",
        views=0,
        downloads=0,
        total_ratings=0,
    ...
    serialized = _serialize_project(db, refreshed)
    serialized["approval_status"] = "pending_admin_approval"
