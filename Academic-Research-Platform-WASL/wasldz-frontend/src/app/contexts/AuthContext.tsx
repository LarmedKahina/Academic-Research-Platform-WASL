...
/** Main app area for each role after login or signup */
export function dashboardPathForRole(role: UserRole): string {
  if (role === 'student') return '/profile/student';
  if (role === 'professor') return '/profile/professor';
  if (role === 'company') return '/profile/company';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signup = async (params: {
    ...
    if (params.role === 'professor') {
      if (params.university) form.append('university', params.university);
    ...
    try {
      ...
      // Do NOT persist the token — account is pending admin approval.
      // The user must wait for approval and then log in normally.
      return u;
    } catch (e) {
      throw new Error(formatApiError(e));
    }
